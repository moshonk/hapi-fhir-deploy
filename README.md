...
- `scripts/echis_seed.rb`: deterministic, shardable household/CHW FHIR dataset generator for the eCHIS progressive benchmark, shaped like the eCHIS dev server: Kenya `Location` hierarchy, `supervision-location` sync tags, CHV/CHA staff, and visits generated by walking the dev server's own Questionnaires (spec 008/010, `docs/echis-data-model.md`).
- `scripts/snapshot_echis_content.rb`: refreshes `benchmarks/echis/content/echis-content.json`, the dev server's Questionnaires, PlanDefinitions, ActivityDefinitions, and country/county Locations that `scripts/echis_seed.rb` loads first.
- `scripts/verify_echis_catchment_data.rb`: post-seed, read-only verification that replays one Community Health Unit's sync calls (Location chain, tag-scoped pulls, roster `_include`, `$everything`, staff chain, Questionnaire canonicals) against a server loaded by `scripts/echis_seed.rb`.
...
- `docs/echis-data-model.md`: eCHIS FHIR dataset model for `scripts/echis_seed.rb`: dev-server-aligned code systems, catchment hierarchy, household roster, visit mix, decision rules, and the opt-in k6 sync operations.
