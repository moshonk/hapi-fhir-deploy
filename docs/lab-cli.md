...
The generated data follows the eCHIS dev server's model (`docs/echis-data-model.md`):
the generator first loads the reference content snapshot in
`benchmarks/echis/content/echis-content.json` (refresh it with
`scripts/snapshot_echis_content.rb`), then writes households whose visits are
generated by walking those Questionnaires. To benchmark the app's sync calls
against the seeded data, pass the same household count to the benchmark:

```sh
ECHIS_SEEDED_HOUSEHOLDS=33333 FHIR_BASE_URL=https://example/fhir \
K6_SCRIPT=benchmarks/k6/echis_load_100.js \
scripts/lab benchmark --profile load --echis-tier T2 --run echis-t2-sync
```

Leave `ECHIS_SEEDED_HOUSEHOLDS` unset to keep the `echis` workload's original,
self-contained operation mix.
