...
| `<ECHIS_SEEDED_HOUSEHOLDS>` | `--households` of the eCHIS dataset `scripts/echis_seed.rb` loaded into the target server, or `0`. When greater than zero, the `echis` workload adds the sync-engine operations (tag-scoped pulls, `Patient/$everything`, known-household reads, PUT+PATCH uploads) against that seeded data; `0` keeps the original self-contained mix. `scripts/lab benchmark --in-cluster` fills it from `ECHIS_SEEDED_HOUSEHOLDS` (default `0`). |
