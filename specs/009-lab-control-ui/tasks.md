...
- [X] T039 [P] [US3] Integration test: triggering `up`/`down`/`expose-fhir`/`expose-prometheus` without `confirmed:true` returns `409` with a `confirmationMessage` naming the concrete consequence; nothing is spawned, in `lab-control-ui/backend/test/integration/confirmation-gate.test.ts` (depends on T037)
...
- [ ] T052 Run `quickstart.md` Scenarios 1-4 end-to-end (Scenario 4 against a real GCP T2 shape, matching `docs/gcp-echis-t3-lab-runbook.md` Step 4's cost/scope profile). Scenarios 1-3 verified against the real built app during implementation (also caught and fixed a real Express 5 `path-to-regexp` bug in the SPA static-file fallback, and a real `ansible-galaxy collection list -p PATH` path-merging bug in the T006 collections check). Scenario 4 requires a real GCP project, credentials, and creates billable resources -- deliberately not run autonomously; needs an operator with GCP access to execute.
