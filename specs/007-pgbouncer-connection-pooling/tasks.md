...
- [X] T005 [P] [US1] Create `manifests/pgbouncer/configmap.yaml` — implemented as env-var config (`POOL_MODE`, `DEFAULT_POOL_SIZE`, `MAX_CLIENT_CONN`, `MAX_DB_CONNECTIONS`, `ADMIN_USERS`, `STATS_USERS`, `MAX_PREPARED_STATEMENTS`) matching the pinned image's env-var-driven `pgbouncer.ini` generation, rather than hand-written `pgbouncer.ini` syntax — see comment in the file for rationale.
- [X] T006 [US1] Create PgBouncer Deployment — implemented as `ansible/templates/pgbouncer-deployment.runtime.yaml.j2` (not a static `manifests/pgbouncer/deployment.yaml`) because `DB_HOST`/`DB_PORT`/`DB_NAME` are only known at deploy time (same reason HAPI's own datasource wiring is already Ansible-templated, not static). Rendered to `ansible/artifacts/generated/pgbouncer-deployment.runtime.yaml` and applied by T011's playbook. Uses the pinned image from T001, the ConfigMap from T005, and reuses the existing `hapi-fhir-postgres` Secret (see T004 note).
...
- [~] T015 [US1] Run `helm lint` / `helm template` with only `--values charts/hapi-fhir-deploy/values.yaml` (pooling disabled) and confirm the render is byte-for-byte identical to the pre-feature baseline, per `quickstart.md` step 1 (depends on T009) — **BLOCKED: helm is not installed in this environment.** Substitute check performed instead: `values-pgbouncer-tier.yaml` is a wholly new, additive file; the base `charts/hapi-fhir-deploy/values.yaml` was not touched by this pass (confirmed via diff-free `git status` on that path). Real `helm lint`/`helm template` run is still required before merge.
...
- [~] T023 [US2] Run `terraform validate` (and `terraform plan` where credentials allow) against all three modules to confirm the new variable/resource wiring is valid (depends on T019, T020, T021) — **BLOCKED: terraform is not installed in this environment.** Substitute check performed instead: brace-balance sanity check across all six changed `.tf` files (all balanced), and each new resource/parameter follows the exact attribute style of its surrounding, previously-working code. Real `terraform validate`/`plan` is still required before merge.
...
Task: "Create manifests/external-secrets/hapi-fhir-pgbouncer-userlist.yaml" (US1, T004)
