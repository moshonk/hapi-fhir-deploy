...
- [X] T027 [P] Add the new "Check PgBouncer connection budget" step to `.github/workflows/ci.yml`, implementing `contracts/connection-budget-formula.md` as a Ruby check parallel to (not replacing) the existing "Check Rev2 baseline decisions" step, including its "no-op when pooled-tier files are absent" behavior (depends on T009, T010, T014, T019, T020, T021). Checks all 5 contract invariants: server-connection budget, `maxReplicaCount` ceiling, pooled `minReplicaCount >= 2`, the pooled/native ScaledObjects sharing identity (same `metadata.name`/`namespace`/`scaleTargetRef`, so they structurally can never coexist as separate objects targeting the same Deployment — the practical, file-level way to enforce contract invariant 4 without a live cluster to apply both against), and the required doc text in `docs/autoscaling.md`. Verified locally: passes against the real repo state, correctly no-ops when the pooled-tier files are absent (tested against a scratch copy with the overlay file removed), and correctly fails with a clear message when a real violation is introduced (tested by bumping `pgbouncer_default_pool_size` to blow the budget). Caught and fixed a real encoding bug during this verification: reading `docs/autoscaling.md` without an explicit UTF-8 encoding fails under a non-UTF-8 locale, since the file contains em-dashes — all `File.read` calls in the new step now pass `encoding: "UTF-8"` explicitly rather than relying on the ambient locale.
- [X] T028 [P] Validate prepared-statement compatibility per `research.md` Decision 3 — pinned PgBouncer >= 1.21 and `MAX_PREPARED_STATEMENTS=200` were already de-risked in the MVP pass. No live cluster available in this sandbox to actually validate the optimized protocol-level-pooling path end-to-end, so rather than assume it works (which Decision 3's own rationale explicitly warns against), applied Decision 3's documented fallback for real: `spring.datasource.hikari.data-source-properties.prepareThreshold: 0` added to `charts/hapi-fhir-deploy/values-pgbouncer-tier.yaml`, disabling PgJDBC server-side prepare entirely (guaranteed-safe, no live validation needed). `MAX_PREPARED_STATEMENTS=200` is now dormant until someone re-enables server-side prepare after live validation. Recorded in `docs/autoscaling.md`'s "Prepared statements" section, including the exact re-enablement/validation steps for that follow-up. Verified: `helm lint`/`helm template` with the overlay applied render correctly and the new value round-trips through both the outer and embedded (`extraConfig`) YAML; the CI guardrail (T027) still passes; the base `values.yaml` render is unaffected (byte-identical, confirmed via a fresh local `helm template` since this change only touches the overlay file).
- [X] T029 Run `specs/007-pgbouncer-connection-pooling/quickstart.md` end-to-end in a lab cluster and record results (depends on T015, T023, T024, T027, T028) — **cannot run in this sandbox** (no live cluster; a local Helm binary was available for steps that don't need one). Status per step: step 1 (zero regression with pooling disabled) already verified via real CI in PR #50 (T015) and re-confirmed locally here; step 2 (Terraform-enforced connection limit) blocked, needs real cloud provisioning; steps 3-4 (deploy pooled tier, verify budget under load) blocked, needs a live cluster; step 5 (CI guardrail) verified locally per T027's notes above; step 6 (prepared-statement compatibility) resolved via T028's applied fallback rather than a live validation run; step 7 (bulk-load/serving separation) the `scripts/lab pause-autoscaling`/`resume-autoscaling` commands were verified via `--dry-run` in PR #59, a live run remains open; step 8 (teardown) not applicable without a real cluster. Live steps 2-4 and 7 remain open action items for whoever has cloud access.
- [X] T030 Sync any changed commands/paths into `AGENTS.md` and `README.md` per the constitution's "keep docs synchronized" requirement. Both already reference PgBouncer/`docs/autoscaling.md` generically without itemizing individual CI steps or overlay-file internals, and this pass added no new user-facing commands or paths (the CI step is internal to `ci.yml`, the Hikari property is internal to the existing overlay file) — no changes needed, matching the T027/T028 changes' actual surface area.
